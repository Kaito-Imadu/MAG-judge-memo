...
- **編集**: ページ一覧パネルのローテーション見出しから `編集` ボタンで `EditRotationModal` を開き、選手の追加・削除・並べ替え・任意位置への挿入・リネーム・団体名変更が可能。追加/削除/挿入時は後続ローテの `startPage` と memoRecord の `pageNumber` を自動シフト。既存ページの採点メモは originalIdx ベースで引き継がれる
